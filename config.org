...
External dependencies (install these system-wide or in venvs):
- =pip install pyright= (LSP server for type checking)
- =pip install ruff-lsp= (Fast linter via LSP)
- Optional: =pip install black= (code formatter)
- Optional: =pip install yapf= (alternative formatter)
...
(use-package
 ...
 :commands lsp
 :config
 ;; Use both pyright (type checking) and ruff (linting)
...
;; LSP handles linting via pyright + ruff-lsp, so we don't need separate
;; flycheck linters. Just point LSP to the right Python interpreter per venv.
(defun my_set_venv (venv)
  "Point LSP to the Python interpreter in VENV."
  (interactive "sChoose a venv from ~/venvs/")
  (message "Setting LSP Python interpreter to %s" venv)
...
- =python3= (Python 3.9+) - elisp-autofmt requires Python
