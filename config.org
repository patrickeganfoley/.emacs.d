...
(use-package
 ...
 ((emacs-lisp-mode . elisp-autofmt-mode)
  (org-mode . my/elisp-autofmt-org-mode))
 :config
 ;; Format elisp blocks in org files
 ;;  (setq elisp-autofmt-python-bin "~/.asdf/shims/python3")

  (defun my/format-elisp-in-org-buffer ()
    "Format all elisp src blocks in current org buffer"
    (interactive)
    (when (derived-mode-p 'org-mode)
      (save-excursion
        (goto-char (point-min))
        (while (re-search-forward "^[ \t]*#\\+begin_src\\s-+elisp" nil t)
...
;; Auto-format on save
(defun my/setup-org-elisp-format ()
  "Setup formatting for config.org"
  (when (and buffer-file-name
             (string-match "config\\.org$" buffer-file-name))
    (add-hook 'before-save-hook #'my/format-elisp-in-org-buffer nil t)))

(add-hook 'org-mode-hook #'my/setup-org-elisp-format)

;; Global keybinding for manual formatting
(global-set-key (kbd "C-c f o") 'my/format-elisp-in-org-buffer)
#+end_src

#+RESULTS:
: my/format-elisp-in-org-buffer
